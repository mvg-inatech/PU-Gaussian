...
class CRNet(torch.nn.Module):
    ...
    """
    def __init__(self, args, up_ratio=4):
...
class SubNetwork(nn.Module):
    ...
    Output:
        up_point: upsampled results, (B, 3, up_ratio * N_input)
    """
    def __init__(self, up_ratio=2, features=None):
        super(SubNetwork, self).__init__()
        self.feature_extractor = Transformer_extractor(128, 64, features=features)
    ...
    def forward(self, points, features=None):
        point_feat = self.feature_extractor(points, features)
...
class Transformer_extractor(nn.Module):
    ...
    """
    def __init__(self, dim_feat, hidden_dim, in_channel=3, features = None):
        ...
        if features is not None:
            self.mlp_2 = MLP_CONV(in_channel=dim_feat*3, layer_dims=[64, dim_feat])
        self.point_transformer = Transformer(dim_feat, dim=hidden_dim)

    def forward(self, points, features=None):
        feature_1 = self.mlp_1(points)
        if features is not None:
            # repeat the features to match the size of the points. for each r replication should follow each other in the last dimension
            r = points.size(2) // features.size(2)
            features =  features.repeat_interleave(r, dim= -1)
            feature_1 = torch.cat([feature_1, features], 1)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        idx_knn = query_knn_point(self.n_knn, pos_flipped, pos_flipped).contiguous()
        key = self.conv_key(x)
        ...
        pos_rel = pos.reshape((b, -1, n, 1)) - grouping_operation(pos.contiguous(), idx_knn)  # b, 3, n, n_knn

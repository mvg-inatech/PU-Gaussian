...
def FPS(pts, fps_pts_num, return_idx=False):
    # input: (b, 3, n)
...
def D_FPS(pts, mean, scale, fps_pts_num, alpha=1.0):
    denisty = map_pts2scale(pts,mean, scale).squeeze(-1)
    # turn scale into density
    # square and add the scale and then take the square root
    denisty = torch.sqrt(torch.sum(denisty ** 2, dim=1, keepdim=True)).squeeze(1)
    sample_idx = density_aware_fps(pts.permute(0, 2, 1).contiguous(), denisty, fps_pts_num, alpha=alpha)
    sample_pts = index_points(pts, sample_idx)
    # (b, 3, fps_pts_num)
    # sample_pts = rearrange(sample_pts, 'b c n -> b n c').contiguous()
    return sample_pts

def map_pts2scale(pts,mean, scale = None, k=1):
    # correct shapes
    selected_mean, idx = get_knn_pts(k= k, pts = mean, center_pts=pts, return_idx=True)
    scale = index_points(scale, idx)

    return scale 

def get_p2p_loss(pred_p2p, sample_pts, gt_pts):
    # input: (b, c, n)
